...
浏览器是如何知道一个网站支持 HTTP/3 (QUIC) 的呢？毕竟 HTTP 默认走 TCP (80/443)。

### a. 服务发现：Alt-Svc

当浏览器首次通过 HTTP/1.1 或 HTTP/2 (基于 TCP) 访问服务器时，服务器可以在响应头中添加一个 `Alt-Svc` (Alternative Service) 字段，来“广而告之”它支持 HTTP/3。
...
*   **h3**: 代表 HTTP/3。
*   **":443"**: 代表 UDP 的 443 端口（虽然也是 443，但是是 UDP 协议）。
*   **ma**: max-age，代表这个信息的有效期（秒）。

**流程**：
1.  浏览器发起第一次 TCP 请求。
2.  服务器返回数据，并带上 `Alt-Svc` 头。
3.  浏览器解析头部，知道“哦，原来你可以走 UDP”。
4.  **下一次**请求时，浏览器会尝试并行发起 QUIC 连接。如果成功，后续就走 HTTP/3；如果不成功（比如 UDP 被防火墙拦截），则继续使用 TCP。

### b. TCP 回退机制 (Fallback)

UDP 协议在某些企业内网或运营商网络中可能会被防火墙拦截或限速。

*   **Race Mechanism (竞速)**：浏览器通常不会死等 QUIC 连接建立。它可能会同时发起 TCP 和 QUIC 连接（或者在 QUIC 连接超时后迅速回退到 TCP）。
*   **Happy Eyeballs**：这是一种算法思想，旨在让双栈（IPv4/IPv6）或多协议（TCP/QUIC）环境下的连接建立更加顺滑，确保用户总能用最快的方式连上服务器，而不会因为某个协议被墙而一直转圈等待。
...
**应用层实现的核心优势**在于**快速迭代与部署**。我们不再需要等待用户更新操作系统内核就能部署最新的网络协议优化，只需要更新浏览器或应用程序即可。
